...
function LeaderLayer:showLeaderLayer(desPosition)
	...
	local layout = cc.CSLoader:createNode("hall/leader/LeaderLayer.csb")
    -- if cc.Director:getInstance():getOpenGLView():getDesignResolutionSize().width == 1280 then
    --     layout:setScale(1.34)
    -- end
	local leader_plane = layout:getChildByName("leader_plane")
    if cc.Director:getInstance():getOpenGLView():getDesignResolutionSize().width == 1280 then
        leader_plane:setScale(1.8)
    end
