...
--获取服务器版本
function GameUpdate:queryVersion(gid,code,bSee,name)
...
function GameUpdate:updateGame(url,version,gid,code)
    ...
    if require("app.HallUpdate"):checkVersion(code,version) == false then
        ...
        if SCENENOW["name"] == "hall.gameScene" then
            SCENENOW["scene"]:selectGame(gid, code)
        end
    ...
    local layerout = SCENENOW["scene"]._scene:getChildByName("content_ly"):getChildByName("content_ly"):getChildByName(strGame)
    if layerout == nil then
        layerout = SCENENOW["scene"]._scene:getChildByName("content_sv"):getChildByName("morecontent_ly"):getChildByName(strGame)
    end
