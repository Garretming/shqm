...
function GroudGameManager:join_freegame(user_id, invitationCode, activityId, history, level, reload)
	...
	local function joinFreeGame_callback(data)
		...
		if netData.returnCode == "0" then
			...
			---加入组局成功
			isGroup = true
			USER_INFO["enter_mode"] = netData["data"]["gameId"]
			USER_INFO["joinGameName"] = netData["data"]["gameName"]
			...
			for k,v in pairs(gameList) do
				if v[1] == gameId then
	                ...
	                if isiOSVerify then
		                ...
		                if v[4] ~= "-1"  then
		                    if v[2] ~= nil then
		                        if require("hall.GameCommon"):getGameVersion(v[2]) == "" then
		                            --本地没有游戏
					...
					if SCENENOW["scene"] then
			    		if SCENENOW["scene"]._scene then
			    			...
			    			if room_plane then
			    				...
			    				btn_create:loadTextures(BackGroupButtonPic_filePath, nil, nil)
			    			end
			    		end
			    	end
...
--查看组局记录（在大厅时检查重连,没游戏就进大厅，有游戏就回游戏）
function GroudGameManager:requestGroupStatusInHallScene(flag,callback)
	...
	local function viewReganizeHistory_callback(data)
	    ...
	    if data_netData ~= nil then
			...
			-- require(CommonTips_filePath):showTips("提示", "", 3, "用户组局历史数据错误")
...
function GroudGameManager:createfreegame(user_id, create_freegame_tbl)
	...
	local function queryUserOnlineGameStatus_callback(data)
		...
		if data == nil then
			require(CommonTips_filePath):showTips("数据异常", "", 2, "用户游戏状态异常，请稍候再试")
	        return
	    end
