...
function headView:addView(view, x, y, width, height, url, userInfo)
	...
	if view then
        ...
        --创建裁切蒙版
        local spStnecil = display.newSprite("hall/view/headView/image/head_bg.png")
        ...
        clipnode:setScale(2)
        clipnode:setPosition(head_iv:getContentSize().width / 2, head_iv:getContentSize().height / 2+4)
    	...
    	if url ~= "" then
    		local url_str = self:getUrlPicture(url)

            -- dump(url_str, "-----url_str-----")

    		if url_str and url_str ~= "" then
    			local imgFullPath = device.writablePath .. url_str

                -- dump(imgFullPath, "-----imgFullPath-----")
                
    			file,fileErr = io.open(imgFullPath)
    			if fileErr == nil then
    				-- head_iv:loadTexture(imgFullPath)

                    local content = display.newSprite(imgFullPath)
    				...
    				local request = network.createHTTPRequest(function (event)
    					...
    					local request_back = event.request
    					local result = event.name
    					if result == "completed" then

    						local code = request_back:getResponseStatusCode()
    						if code == 200 then
    							--请求成功，显示服务端返回的内容
            					local response = request_back:getResponseString()
            					--保存下载数据到本地文件，如果不成功，重试30次。
					            local times = 1
					            while (not request_back:saveResponseData(imgFullPath)) and times < 30 do
					                times = times + 1
					            end
    						...
    						dump(request_back:getErrorCode(), "-----头像请求-----")
    						dump(request_back:getErrorMessage(), "-----头像请求-----")
    					end
