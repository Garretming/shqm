...
--加载骰子动画资源
local shaiziPin_map = require("js_majiang_3d.pintu.MajiangshuaiziMap")
...
function AniUtils:shuaiZi(point)
	...
	local pannel = display.newSprite("js_majiang_3d/image/shaiziPin.png"):addTo(node)
	...
	bm.SchedulerPool:loopCall(function()
		...
		if bm.shaizi_count == 17 then
			...
			local pannel_one = display.newSprite("js_majiang_3d/image/".."dian"..tostring(one)..".png"):addTo(node)
			pannel_one:pos(-60,0)

			local pannel_two =  display.newSprite("js_majiang_3d/image/".."dian"..tostring(two)..".png"):addTo(node)
			pannel_two:pos(60,0)
