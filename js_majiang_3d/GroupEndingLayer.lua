...
--显示一局结算的结算界面
function GroupEndingLayer:showGroupResult(pack,disaband_flag)
	--dump(pack, "-----一局结算信息-----")
	--dump(ZZMJ_ROOM, "显示房间信息")

	--获取总结算界面
	local  TotalResultCsb_filePath = TotalResultCsb_filePath or "js_majiang_3d/GameResult.csb"

    local s = cc.CSLoader:createNode(TotalResultCsb_filePath)
    -- s:setScale(0.75)
    ...
    if #playerlist > 0 then
    	...
    	for k,v in pairs(playerlist) do
	    	...
	    	--定义结果分项
	    	local ResultItemCsb_filePath = ResultItemCsb_filePath or "js_majiang_3d/GameResult_Item.csb"

	    	local result_Item = cc.CSLoader:createNode(ResultItemCsb_filePath)
	    	result_Item:setLocalZOrder(zOrder)
	    	zOrder = zOrder - 1
	    	content_sv:addChild(result_Item)

	    	local item_ly = result_Item:getChildByName("item_ly")
	    	item_ly:setPosition(300 * (k - 1), content_sv:getContentSize().height)
	    	...
	    	require("hall.GameCommon"):getUserHead(user_info.photo_url, v.uid, user_info.sex, head_im, 88, false)
	    	...
	    	name_tt:setString(require("hall.GameCommon"):formatNick(user_info.nick_name, 4))
	    	...
	    	xhdp_ly:setPosition(xhdp_ly:getPosition().x, xhdp_ly:getPosition().y + 90)
	    	...
	    	dhjp_ly:setPosition(dhjp_ly:getPosition().x, dhjp_ly:getPosition().y + 90)
	    	...
	    	if chips == - 2000 or disaband_flag == true then
	    		chips = 0
	    	end
        ...
        function(sender,event)
            ...
            if event == TOUCH_EVENT_ENDED then
                ...
                elseif device.platform ~= "windows" then
                	require("hall.common.ShareLayer"):shareGroupResultForIOS()
                end
